#!/bin/bash
#SBATCH --job-name=DPSK_CEWE
#SBATCH --partition=a100
#SBATCH --nodes=1                       
#SBATCH --gres=gpu=1                   # ⬅️ 每节点申请2个GPU
#SBATCH --ntasks-per-node=1
#SBATCH --time=48:00:00
#SBATCH --output=/mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/logs/DPSK_CEWE_%j_%t.out

# ✅ 1. 激活你的 Conda 环境
source ~/.bashrc
conda activate GPG

# export CUDA_HOME=/usr/local/cuda-12.6
# export PATH=$CUDA_HOME/bin:$PATH
# export LD_LIBRARY_PATH=$CUDA_HOME/lib64:$LD_LIBRARY_PATH

# ✅ 2. 分布式训练相关环境变量（自动设置）
export MASTER_ADDR=$(scontrol show hostnames $SLURM_JOB_NODELIST | head -n 1)
export MASTER_PORT=$((29500 + RANDOM % 1000))
export RANK=$SLURM_NODEID
export NUM_GPUS_PER_NODE=2
export WORLD_SIZE=$SLURM_NNODES
export GPUS=$((WORLD_SIZE * NUM_GPUS_PER_NODE))

echo "RANK: $SLURM_NODEID"
echo "GPUS = $GPUS"
echo "MASTER_ADDR=$MASTER_ADDR"

nvidia-smi


# ✅ 3. 设置 python 包路径（如有必要）
export PYTHONPATH=/mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/src:$PYTHONPATH

# ✅ 4. 启动训练
accelerate launch \
  --config_file /mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/recipes/accelerate_configs/zero2.yaml \
  --num_machines $WORLD_SIZE \
  --machine_rank $RANK \
  --num_processes $GPUS \
  --main_process_ip $MASTER_ADDR \
  --main_process_port $MASTER_PORT \
  /mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/src/open_r1/grpo.py \
  --config /mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/recipes/DeepSeek-R1-Distill-Qwen-1.5B/grpo/config_demo.yaml \
  --output_dir /mnt/fast/nobackup/scratch4weeks/ly0008/xch/code/CEWE_/output_logs/CEWE/GRPO/DeepSeek-R1-Distill-Qwen-1.5B \
