...
class GPGTrainer(GRPOTrainer):
    def __init__(
        ...
        self.control.should_evaluate = False  # Disable evaluation by default, as we will do it manually.
        
        self.per_token_logps = None
        self.completion_lengths = 1
    ...
    def _generate_and_score_completions(
        ...
        while n_gen <= max_gen:
            ...
            with torch.no_grad():
                ...
                self.completion_lengths = completion_mask.sum(dim=1)

                with unwrap_model_for_generation(
                    self.model_wrapped, self.accelerator, gather_deepspeed3_params=self.args.ds3_gather_for_generation
                ) as unwrapped_model:
                    self.per_token_logps = self._get_per_token_logps(self.model, prompt_completion_ids, attention_mask, logits_to_keep)
                    print("GPG - self.per_token_logps:", self.per_token_logps.size())
            ...
            for i, (reward_func, reward_processing_class) in enumerate(
                ...
                with profiling_context(self, reward_func_name):
                    if isinstance(
                        ...
                        reward_kwargs["per_token_logps"] = self.per_token_logps
                        reward_kwargs["token_level_length"] = self.completion_lengths
