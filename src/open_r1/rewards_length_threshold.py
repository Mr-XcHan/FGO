...
from grader import grade_answer, extract_answer, extract_answer_gsm8k
...
def accuracy_reward(completions,  **kwargs):
    ...
    rewards = []
    for content, sol in zip(contents, kwargs["answer"][0]):
        gold_parsed = parse(
            sol,
...
#             if grade_answer(answer, kwargs["answer"][0]):
#                 rewards.append(1.0)
#             else:
#                 rewards.append(0.0)
#     return rewards
...
def accuracy_reward_gsm8k(completions,  **kwargs):
    ...
    for completion in completions:
        if completion is None:
            ...
            # Check if the answer is correct
            answer = extract_answer(completion[0]["content"])
            if grade_answer(answer, extract_answer_gsm8k(kwargs["answer"][0])):
                rewards.append(1.0)
            else:
                rewards.append(0.0)
    return rewards
...
def _extract_content(completion):
    try:
        return completion[0][0]["content"]  # 原始预期格式
    except (TypeError, IndexError, KeyError):
        try:
            return completion[0]["content"]  # 一层 list
        except (TypeError, IndexError, KeyError):
            if isinstance(completion, str):
                return completion  # 是 string
            return ""
...
def format_reward(completions, **kwargs):
    ...
    if  isinstance(completions[0],(list,)):
        completion_contents = [_extract_content(completion) for completion in completions]
    else:
    ...
    with torch.no_grad():
        ...
        for completion in completions:
            if completion is None:
                ...
                print("answer:", answer)
                print("ground_solution:",kwargs["solution"][0])
                print('ground_truth:',extract_answer(kwargs["solution"][0]))
                
                if grade_answer(answer, extract_answer(kwargs["solution"][0])):  # this for mathlighteval
                # if grade_answer(answer, extract_answer_gsm8k(kwargs["solution"][0])):
                    rewards.append(1.0)
                else:
                    rewards.append(-1.0)
        
        rewards_tensor = torch.tensor(rewards, dtype=torch.float32)
        weights = torch.ones(len(rewards), dtype=torch.float32)

        print("original rewards:", rewards)

        if kwargs['per_token_logps'] != None:
            logps = [logps.detach().float() for logps in kwargs['per_token_logps']]
            length_token = kwargs['token_level_length'].detach().cpu().int() # token-level lengths
            entropys = []

            for logp, L in zip(logps, length_token):
                valid_logp = logp[:L]  
                entropy = -torch.sum(torch.exp(valid_logp) * valid_logp).item()
                entropys.append(entropy)
            entropys = torch.tensor(entropys)
            print("entropys:", entropys)

            # lengths = torch.tensor([len(logp) for logp in logps]) # Padding token-level lengths
            # lengths_ = torch.tensor([len(content) for content in contents]) # character-level lengths

            for reward_element in torch.unique(rewards_tensor):
                idx = (rewards_tensor == reward_element).nonzero(as_tuple=True)[0]  # 找到当前组的索引
                entropys_sub = entropys[idx]
                lengths_sub = length_token[idx]

                lengths_sub[lengths_sub < 0.1 * 2048] = 0.1 * 2048
                print("length_token:", lengths_sub)
                ...
                weights[idx] = weights_sub


            wandb.log({
            "Correctness/mean": rewards_tensor.mean().item(),
            "Correctness":wandb.Histogram(rewards_tensor.cpu().numpy())
            })

            wandb.log({
            "Entropys/mean": entropys.mean().item(),
            "Entropys":wandb.Histogram(entropys.cpu().numpy())
            })

            wandb.log({
            "Lengths/mean": length_token.float().mean().item(),
            "Lengths":wandb.Histogram(length_token.cpu().numpy())
            })
        
        rewards_tensor = rewards_tensor * weights

        print("Weight reward:", rewards_tensor)
    return rewards_tensor.cpu().tolist()


# # for open-s1数据集，但是gold_parsed 提取还存在问题.
# def cewe(completions, **kwargs):
#     contents = [completion[0]["content"] for completion in completions]
#     rewards = []
#     for content, sol in zip(contents, kwargs["answer"]):
#         gold_parsed = parse(
#             sol,
...
#         logps = [logps.detach().float() for logps in kwargs['per_token_logps']]
#         length_token = kwargs['token_level_length'].detach().cpu().int() # token-level lengths
#         print("length_token:", length_token)
#         entropys = []
...
#             completion_length_scale = lengths_sub.float().mean()
#             entropy_scale = entropys_sub.mean()
#             print("completion_length_scale:", completion_length_scale)
#             print("entropy_scale:", entropy_scale)
...
#                 weights_raw = (completion_length_scale * entropy_scale) / (lengths_sub * entropys_sub + 1e-6)
#             else:
#                 weights_raw = (completion_length_scale * entropys_sub) / (lengths_sub * entropy_scale + 1e-6)
